...
(define (SETTLER STR)
    (let LOOP [ (C (char->list STR)) (TEMP_L empty) (TEMP_S "") ]
        ...
        (cond
            [   (empty? C) (set! STR (if (equal? "" TEMP_S) TEMP_L (reverse(cons TEMP_S (reverse TEMP_L)))))  STR   ];  (println STR) ]
            ...
            [   (or (list? (first E)) );(equal? "|"(last(first E)))  (equal? "*"(last(first E)))  (equal? "+"(last(first E))) )
                (cond
                    ...
                    [   (or (equal? "*"(last(first E))) (equal? "+"(last(first E))) )
                        (if (equal? "*"(last(first E)))
                            (let() (set! E (cons(reverse(rest(reverse (first E)))) (rest E)) ) ; remove the symbol "*"
                                   ;(println E)
                                   (LOOP S (rest E))
...
;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||| TESTS ||||||||||||||||||||||||||||||||||||||||||||||||
 ...
 ;(FSM2 "go stop wait"  "( go | stop | wait | right | left )*")

 ;(FSM2 "go wait wait left wait go wait stop" "((go|left|right)* wait)+ stop" )
 ;(FSM2 "go wait wait stop" "((go|left) wait)* stop" )